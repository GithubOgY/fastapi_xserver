...
- [x] **監査ログ**: 管理者操作の記録（2026/01/02 完了）
  - Phase 1: 基盤構築完了
    - AuditLogモデル実装（database.py）
    - 監査ログヘルパー関数実装（main.py）
    - スタートアップマイグレーション実装
  - Phase 2: 認証エンドポイント統合完了
    - ログイン（LOGIN_SUCCESS / LOGIN_FAILED）
    - ログアウト（LOGOUT）
    - ユーザー登録（REGISTER）
  - Phase 3: 管理者操作統合完了
    - ユーザー削除（USER_DELETE）
  - Phase 4: モデレーション統合完了
    - 投稿削除（COMMENT_DELETE_ADMIN / COMMENT_DELETE_OWNER）
  - Phase 5: 管理者UI完了
    - `/admin/audit-logs` エンドポイント実装
    - フィルタリング・ページネーション対応
    - 詳細情報モーダル表示
