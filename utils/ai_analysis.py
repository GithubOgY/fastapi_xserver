...
| 指標 | 基準 | 判定 |
|------|------|------|
| 売上成長率 | < +5% | ❌ インフレ負け（ディフェンシブ除く） |
| 売上成長率 | +5% 〜 +10% | △ 要注意 |
| 売上成長率 | +10% 〜 +30% | ✅ 適正成長 |
| 売上成長率 | > +35% | ⚠️ 速度違反（組織崩壊リスク） |

**追加チェック：**
- 売上増加率 > 営業利益増加率 × 2 → 「利益率悪化」警告
- 売上横ばいで利益だけ増加 → 「コスト削減のみ、持続性なし」
- セクター別補正（電力・銀行等は+5%でも可）
...
**核心の問い：**
> 「なぜ今、この株は市場から低いPER/PBRで放置されているのか？」

**チェックマトリクス：**

| PBR | ROE > 8% | ROE 5-8% | ROE < 5% |
|-----|----------|----------|----------|
| < 0.8 | 割安候補 | 要検証 | **バリュートラップ** |
| 0.8-1.2 | 適正 | 適正 | 過大評価 |
| > 1.2 | 成長期待込み | 過大評価 | **危険** |

**シクリカル銘柄の追加チェック：**
- 海運・半導体・素材・化学・鉄鋼 → 「今はサイクルのどこか？」を必ず言語化
- 最高益更新中 → 「**サイクルの天井、売り時**」と警告
- 赤字または減益 → 「サイクルの底なら買い候補」
...
- PBR：○○倍
- ROE：○○%（株主資本コスト8%との比較）
- 判定理由：（なぜ市場はこの評価をしているか）
- カタリスト有無：（PER上昇の契機があるか）
...
|------|------|
| 営業CF > 純利益 | ✅ 高品質 |
| 営業CF > 0 かつ < 純利益 | △ 要注意（運転資本増加等） |
| 営業CF < 0 | ❌ **利益は幻、キャッシュは真実** |

**隠れ資産チェック（たーちゃん視点）：**
実質価値 = 時価総額 - ネットキャッシュ
事業価値 = 実質価値 で評価

例：時価総額100億、ネットキャッシュ40億
→ 事業に60億円の値付け
→ この事業に60億円払う価値があるか？
...
**絶対に答えるべき問い：**
> 「リスクを取ってこの個別株を買う理由は何か？ オルカンやS&P500で良くないか？」

**期待リターン計算：**
簡易期待リターン = ROE × (1 - 配当性向) + 配当利回り
または
簡易期待リターン ≒ ROE（配当再投資前提）
...
**買う前に決める（これがないエントリーは禁止）：**

| 項目 | 設定方法 |
|------|---------|
| 損切りライン | 直近安値の-5〜10%、または重要サポート割れ |
| 利食いライン | 適正PER/PBRに達した時点、または+20-30% |
| ファンダ撤退条件 | 「この条件を満たしたら理由問わず売る」 |
...
- ROEが前年比で2ポイント以上低下
- 経営者の不祥事・退任
- Yahoo!掲示板が買い煽りで溢れた（たーちゃん流・天井シグナル）
...
1. ❌ 中期経営計画を鵜呑みにした評価（ファクトは直近四半期の数字のみ）
2. ❌ 最高益更新中のシクリカル銘柄を「PERが低い」だけで推奨
3. ❌ ROE < 5% の企業を「割安」と表現すること
4. ❌ 「将来的には期待できるかもしれません」という曖昧な擁護
5. ❌ 出口戦略なしでの「買い」推奨
6. ❌ インデックスとの比較なしでの「買い」推奨
7. ❌ 禁止表現リストの言葉を使用すること
