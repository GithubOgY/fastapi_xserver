...
- 売上が増加している場合：新規事業の拡大、既存事業の成長、M&A、価格上昇など、どの要因が考えられるか
- 営業利益率が改善している場合：コスト削減、高付加価値商品へのシフト、規模の経済など、どの要因が考えられるか
- 有利子負債が減少している場合：返済努力、資産売却、増資、キャッシュフロー改善など、どの要因が考えられるか
- キャッシュフローが悪化している場合：在庫増加、売掛金の増加、設備投資、運転資金の圧迫など、どの要因が考えられるか
- これらの背景推測は、summaryやweaknesses/strengthsに必ず含めること

## ダッシュボードの構成（全5つのグラフセクション）
...
**重要**: 画像内に上記5つのグラフセクションがすべて含まれていることを確認し、それぞれを漏れなく分析すること。
...
- **背景推測**: この乖離の背景を必ず推測すること（例：在庫の積み増し、売掛金の増加、前払金の増加、会計上の利益計上タイミングのずれなど）
- この条件に該当する企業は、他の指標が良好でも評価を大幅に下げること（最低でもC判定以下）
- 背景推測の内容はweaknessesに必ず記載すること

### 第2段階: 基本的な財務分析と背景推測
- 売上/営業利益グラフから成長性と収益性を評価
- 財務健全性指標（自己資本比率、有利子負債、ROE、ROA）を確認
- 成長性分析（売上CAGR/EPS CAGR）を評価
- **重要**: 各指標の変化の背景を必ず推測すること
  - 売上成長の背景：新規事業、既存事業の拡大、M&A、価格戦略など
  - 利益率変化の背景：コスト構造の変化、商品ポートフォリオの変化、競争環境の変化など
  - 有利子負債変化の背景：財務戦略の変更、返済計画、資金調達方針の変更など
  - ROE/ROA変化の背景：収益性の改善、資産効率の向上、財務レバレッジの変化など
...
- **背景推測**: これらの兆候が見られる場合、経営層が好業績に慢心し、コスト意識が低下している可能性、または将来の業績悪化を隠すための「見せかけ」の可能性を推測すること
...
- **背景推測の必須化**: 数値の変化を読み取ったら、必ずその背景や理由を推測し、summaryやweaknesses/strengthsに記載すること。単に「売上が増加」と書くのではなく、「売上が増加（新規事業の拡大が背景と推測）」のように背景を含めること
- **足切り条件の優先適用**: 第1段階（CFチェック）や最終判断（インデックス比較）で該当する場合は、他の指標が良好でも評価を下げること
- お世辞は不要。問題点は遠慮なく指摘すること
- 業界特有の事情は考慮しつつも、投資家視点で厳格に評価
- 曖昧な表現は避け、明確な判断を示すこと
- strengthsとweaknessesは各最大3項目まで、簡潔に（ただし背景推測を含めること）
- recommendationsは投資判断の具体的根拠を3つ（背景推測に基づく判断を含めること）
- one_linerはこの銘柄の本質を的確に表現する一言（背景推測を含めた洞察を反映すること）
